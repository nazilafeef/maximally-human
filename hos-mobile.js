...
(function () {
  ...
  function init(HOS) {
    ...
    if (isTouch()) {
      /* The engine binds touchstart for a.xref only, and dismissal there
         depends on :hover, which a finger never produces. Cover every
         trigger, and give the sheet a scrim to tap away. */
      document.addEventListener('click', function (e) {
        var t = e.target.closest(TRIGGERS);
        if (t) {
          // let the engine's own handler render, then dress it for touch
          setTimeout(function () { if (!pop.hidden) addScrim(); }, 0);
          return;
        }
